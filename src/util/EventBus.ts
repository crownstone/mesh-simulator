class EventBusClass {
  ...
  on(topic, callback) {
    if (!(topic)) {
      console.error('Attempting to subscribe to undefined topic:', topic);
      return;
    }
    if (!(callback)) {
      console.error('Attempting to subscribe without callback to topic:', topic);
      return;
    }
    ...
    // generate unique id
    let id = Math.floor(Math.random()*1e8).toString(36)
